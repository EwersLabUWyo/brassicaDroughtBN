...
The network structure, defining the dependencies in the network didn't change, but distributions did based on our observation of, or conditioning on, PR. While the network structure might shift radically when subsetting the data, the conditional probability tables are geared specifically towards queries which subset the data. 
